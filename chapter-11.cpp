...
int main()
{
    ...
    float aperture = 2.0;
    camera cam(lookfrom, lookat, vec3(0, 1, 0), 30, float(nx / ny), aperture, dist_to_focus);
    for (int j = ny - 1; j >= 0; j--)
    {
        for (int i = 0; i < nx; i++)
        {
            ...
            col /= float(ns);
            // gamma correction , example: power 1/gamma
    ...
    stbi_write_png("chapter-11.png", nx, ny, channel_num, data, 0);
}
